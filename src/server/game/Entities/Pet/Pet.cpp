    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (isControlled())
            {
                ...
                {
                    TC_LOG_ERROR("entities.pet", "Pet %u is not pet of owner %s, removed", GetEntry(), m_owner->GetName());
    ...
    if (l_Owner && isPet() && l_Owner->IsPlayer())
    {
        switch (l_Owner->getClass())
        {
            ...
            case CLASS_HUNTER:
                ...
                break;
            default:
                TC_LOG_ERROR("entities.pet", "Unknown type pet %u is summoned by player class %u", GetEntry(), l_Owner->getClass());
                break;
        }
    }
