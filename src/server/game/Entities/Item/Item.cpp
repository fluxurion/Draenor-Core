    ...
    for (auto setSpell : setSpells)
    {
        ...
        for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y)
        {
            if (!eff->spells[y])                             // free slot
            {
                ...
                if (!spellInfo)
                {
                    TC_LOG_ERROR("entities.player.items", "WORLD: unknown spell id %u in items set %u effects", setSpell->SpellID, setid);
                    break;
                }
...
Item::~Item()
{
    ...
    if (Player * plr = GetOwner())
    {
        if (RemoveItemByDelete(plr, this))
        {
            ...
            TC_LOG_ERROR("server.worldserver", "Item %u on player guid %u is in destructor, and pointer is still referenced in player's data ...", GetEntry(), plr->GetGUIDLow());
            TC_LOG_ERROR("server.worldserver", "Stack Trace : %s", l_Trace.c_str());
        }
    }
}
