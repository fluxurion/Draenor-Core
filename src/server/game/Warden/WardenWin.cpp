...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    {
        TC_LOG_WARN(LOG_FILTER_WARDEN, "%s failed hash reply. Action: %s", _session->GetPlayerName(false).c_str(), Penalty().c_str());
        return;
    }
...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    {
        buff.rpos(buff.wpos());
        TC_LOG_WARN(LOG_FILTER_WARDEN, "%s failed checksum. Action: %s", _session->GetPlayerName(false).c_str(), Penalty().c_str());
        return;
    }
    ...
    // TIMING_CHECK
    {
        ...
        if (result == 0x00)
        {
            TC_LOG_WARN(LOG_FILTER_WARDEN, "%s failed timing check. Action: %s", _session->GetPlayerName(false).c_str(), Penalty().c_str());
            return;
        }
    ...
    if (checkFailed > 0)
    {
        ...
        TC_LOG_WARN(LOG_FILTER_WARDEN, "%s failed Warden check %u. Action: %s", _session->GetPlayerName(false).c_str(), checkFailed, Penalty(check).c_str());
    }
