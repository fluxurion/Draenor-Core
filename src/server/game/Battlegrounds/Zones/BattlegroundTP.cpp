...
void BattlegroundTP::RespawnFlag(uint32 Team, bool captured)
{
    if (Team == ALLIANCE)
    {
        TC_LOG_DEBUG(LOG_FILTER_BATTLEGROUND, "Respawning Alliance flag");
        m_FlagState[TEAM_ALLIANCE] = BG_TP_FLAG_STATE_ON_BASE;
    }
    else
    {
        TC_LOG_DEBUG(LOG_FILTER_BATTLEGROUND, "Respawning Horde flag");
...
void BattlegroundTP::RespawnFlagAfterDrop(uint32 team)
{
    ...
    if (object)
        ...
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "unknown dropped flag bg, guid: %u", GUID_LOPART(GetDroppedFlagGUID(team)));
...
bool BattlegroundTP::SetupBattleground()
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BatteGroundTP: Failed to spawn some objects Battleground not created!");
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BatteGroundTP: Failed to spawn Alliance spirit guides! Battleground not created!");
        return false;
    }
    ...
    if (!sg || !AddSpiritGuide(TP_SPIRIT_ALLIANCE, sg->x, sg->y, sg->z, sg->o, ALLIANCE))
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BatteGroundTP: Failed to spawn Alliance start spirit guides! Battleground not created!");
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BatteGroundTP: Failed to spawn Horde spirit guides! Battleground not created!");
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BatteGroundTP: Failed to spawn Horde start spirit guides! Battleground not created!");
        return false;
    }
