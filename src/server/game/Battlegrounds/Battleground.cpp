    ...
    if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL)
    {
        ...
        for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
            {
                ...
                {
                    TC_LOG_DEBUG(LOG_FILTER_BATTLEGROUND, "BATTLEGROUND: Sending %s back to start location (map: %u) (possible exploit)", player->GetName(), GetMapId());
...
{
#ifdef CROSS
    TC_LOG_ERROR("server.worldserver", "Battleground::AddPlayer: instance id %u, bg status: %u, player guid %u", GetInstanceID(), GetStatus(), player->GetRealGUIDLow());
...
#endif /* CROSS */
    // Log
    TC_LOG_INFO(LOG_FILTER_BATTLEGROUND, "BATTLEGROUND: Player %s joined the battle.", player->GetName());
}
