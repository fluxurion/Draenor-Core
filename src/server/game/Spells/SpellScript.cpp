...
bool AuraScript::_Validate(SpellInfo const* entry)
{
    ...
    for (std::list<EffectPeriodicHandler>::iterator itr = AfterEffectPeriodic.begin(); itr != AfterEffectPeriodic.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            TC_LOG_ERROR("scripts", "Spell `%u` Effect `%s` of script `%s` did not match dbc effect data - handler bound to hook `AfterEffectPeriodic` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
