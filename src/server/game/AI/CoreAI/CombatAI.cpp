...
{
    if (!me->m_spells[0])
        TC_LOG_ERROR("server.worldserver", "ArcherAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
{
    if (!me->m_spells[0])
        TC_LOG_ERROR("server.worldserver", "TurretAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
