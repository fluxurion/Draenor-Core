    ...
    try
    {
        switch (opcode)
        {
            ...
            default:
            {
                ...
                {
                    //TC_LOG_ERROR("network.opcode", "No defined handler for opcode %s sent by %s", GetOpcodeNameForLogging(new_pct->GetOpcode(), WOW_CLIENT_TO_SERVER).c_str(), m_Session->GetPlayerName(false).c_str());
                    return 0;
                }
