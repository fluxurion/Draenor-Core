...
namespace MMAP
{
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        {
            dtFreeNavMesh(mesh);
            TC_LOG_ERROR("server.worldserver", "MMAP:loadMapData: Failed to initialize dtNavMesh for mmap %04u from file %s", mapId, fileName.c_str());
            return false;
        }
    ...
    bool MMapManager::loadMap(const std::string& /*basePath*/, uint32 mapId, int32 x, int32 y)
    {
        ...
        {
            TC_LOG_ERROR("server.worldserver", "MMAP:loadMap: Bad header in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        {
            TC_LOG_ERROR("server.worldserver", "MMAP:loadMap: %04u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        if (!result)
        {
            TC_LOG_ERROR("server.worldserver", "MMAP:loadMap: Bad header or data in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        TC_LOG_ERROR("server.worldserver", "MMAP:loadMap: Could not load %04u%02i%02i.mmtile into navmesh", mapId, x, y);
        dtFree(data);
        return false;
    }
    ...
    PhasedTile* MMapManager::LoadTile(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (!file)
        {
            ...
            //TC_LOG_DEBUG("maps", "phase", "MMAP:LoadTile: Could not open mmtile file '%s'", fileName);
            return NULL;
        }
        ...
        {
            TC_LOG_ERROR("server.worldserver", "MMAP:LoadTile: Bad header in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            delete pTile;
            return nullptr;
        }
        ...
        {
            TC_LOG_ERROR("server.worldserver", "MMAP:LoadTile: %04u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        if (!result)
        {
            TC_LOG_ERROR("server.worldserver", "MMAP:LoadTile: Bad header or data in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            delete pTile;
            return nullptr;
        }
    ...
    {
        TC_LOG_DEBUG("maps", "MMAP:LoadPhaseTiles: Loading phased mmtiles for map %u, x: %i, y: %i", phasedMapData->first, x, y);
        ...
        for (uint32 phaseMapId : phasedMapData->second)
        {
            ...
            {
                TC_LOG_DEBUG("maps", "MMAP:LoadPhaseTiles: Loaded phased %04u%02i%02i.mmtile for root phase map %u", phaseMapId, x, y, phasedMapData->first);
    ...
    {
        TC_LOG_DEBUG("maps", "MMAP:UnloadPhaseTile: Unloading phased mmtile for map %u, x: %i, y: %i", phasedMapData->first, x, y);
        ...
        for (uint32 phaseMapId : phasedMapData->second)
        {
            ...
            {
                TC_LOG_DEBUG("maps", "MMAP:UnloadPhaseTile: Unloaded phased %04u%02i%02i.mmtile for root phase map %u", phaseMapId, x, y, phasedMapData->first);
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (dtStatusFailed(mmap->navMesh->removeTile(tileRef, NULL, NULL)))
        {
            ...
            TC_LOG_ERROR("server.worldserver", "MMAP:unloadMap: Could not unload %04u%02i%02i.mmtile from navmesh", mapId, x, y);
            abort();
        }
        else
        {
    ...
    dtNavMeshQuery const* MMapManager::GetNavMeshQuery(uint32 mapId, uint32 instanceId, TerrainSet swaps)
    {
        ...
        if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
        {
            ...
            {
                dtFreeNavMeshQuery(query);
                TC_LOG_ERROR("server.worldserver", "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId %04u instanceId %u", mapId, instanceId);
                return NULL;
            }
    ...
    void MMapData::RemoveSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        {
            TC_LOG_DEBUG("maps", "MMapData::RemoveSwap: mmtile %04u[%02i, %02i] unload skipped, due to not loaded", swap, x, y);
            return;
        }
        ...
        if (dtStatusFailed(navMesh->removeTile(loadedTileRefs[packedXY], NULL, NULL)))
            TC_LOG_ERROR("server.worldserver", "MMapData::RemoveSwap: Could not unload phased %04u%02i%02i.mmtile from navmesh", swap, x, y);
        else
        {
            TC_LOG_DEBUG("maps", "MMapData::RemoveSwap: Unloaded phased %04u%02i%02i.mmtile from navmesh", swap, x, y);

            // restore base tile
            if (dtStatusSucceed(navMesh->addTile(_baseTiles[packedXY]->data, _baseTiles[packedXY]->dataSize, 0, 0, &loadedTileRefs[packedXY])))
            {
                TC_LOG_DEBUG("maps", "MMapData::RemoveSwap: Loaded base mmtile %04u[%02i, %02i] into %04i[%02i, %02i]", _mapId, x, y, _mapId, header->x, header->y);
            }
            else
                TC_LOG_ERROR("server.worldserver", "MMapData::RemoveSwap: Could not load base %04u%02i%02i.mmtile to navmesh", _mapId, x, y);
        }
        ...
        if (loadedPhasedTiles[swap].empty())
        {
            ...
            TC_LOG_DEBUG("maps", "MMapData::RemoveSwap: Fully removed swap %u from map %u", swap, _mapId);
        }
    }
    ...
    void MMapData::AddSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        {
            TC_LOG_DEBUG("maps", "MMapData::AddSwap: phased mmtile %04u[%02i, %02i] load skipped, due to not loaded base tile on map %u", swap, x, y, _mapId);
            return;
        }
        if (loadedPhasedTiles[swap].find(packedXY) != loadedPhasedTiles[swap].end())
        {
            TC_LOG_DEBUG("maps", "MMapData::AddSwap: WARNING! phased mmtile %04u[%02i, %02i] load skipped, due to already loaded on map %u", swap, x, y, _mapId);
            return;
        }
        ...
        if (!oldTile)
        {
            TC_LOG_DEBUG("maps", "MMapData::AddSwap: phased mmtile %04u[%02i, %02i] load skipped, due to not loaded base tile ref on map %u", swap, x, y, _mapId);
            return;
        }
        ...
        {
            TC_LOG_ERROR("server.worldserver", "MMapData::AddSwap: Could not unload %04u%02i%02i.mmtile from navmesh", _mapId, x, y);
            delete pt;
        }
        else
        {
            TC_LOG_DEBUG("maps", "MMapData::AddSwap: Unloaded %04u%02i%02i.mmtile from navmesh", _mapId, x, y);
            ...
            {
                TC_LOG_DEBUG("maps","MMapData::AddSwap: Loaded phased mmtile %04u[%02i, %02i] into %04i[%02i, %02i]", swap, x, y, _mapId, header->x, header->y);
            }
            else
                TC_LOG_ERROR("server.worldserver", "MMapData::AddSwap: Could not load %04u%02i%02i.mmtile to navmesh", swap, x, y);
        }
    }
