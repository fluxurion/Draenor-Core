...
public:
    ...
    struct instance_old_hillsbrad_InstanceMapScript : public InstanceScript
    {
        ...
        Player* GetPlayerInMap()
        {
            ...
            TC_LOG_DEBUG("scripts", "TSCR: Instance Old Hillsbrad: GetPlayerInMap, but PlayerList is empty!");
            return NULL;
        }
        ...
        void SetData(uint32 type, uint32 data)
        {
            ...
            if (!player)
            {
                TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: SetData (Type: %u Data %u) cannot find any player.", type, data);
                return;
            }
            ...
            switch (type)
            {
                case TYPE_BARREL_DIVERSION:
                {
                    if (data == IN_PROGRESS)
                    {
                        ...
                        TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: go_barrel_old_hillsbrad count %u", mBarrelCount);
                ...
                case TYPE_THRALL_EVENT:
                {
                    if (data == FAIL)
                    {
                        if (mThrallEventCount <= 20)
                        {
                            ...
                            TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: Thrall event failed %u times. Resetting all sub-events.", mThrallEventCount);
                        ...
                        m_auiEncounter[1] = data;
                    TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: Thrall escort event adjusted to data %u.", data);
                    break;
                }
                case TYPE_THRALL_PART1:
                    m_auiEncounter[2] = data;
                    TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: Thrall event part I adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART2:
                    m_auiEncounter[3] = data;
                    TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: Thrall event part II adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART3:
                    m_auiEncounter[4] = data;
                    TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: Thrall event part III adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART4:
                    m_auiEncounter[5] = data;
                     TC_LOG_DEBUG("scripts", "TSCR: Old Hillsbrad: Thrall event part IV adjusted to data %u.", data);
                    break;
            }
        }
